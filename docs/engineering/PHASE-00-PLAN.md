...
| `FND-020` | Partial | Versioned PR static/history/contracts, real PostgreSQL/NATS, CodeQL, and supply-chain jobs call repository-owned commands; local equivalents and seeded failures pass in [S07 evidence](../../evidence/phase-00/supply-chain/S07-ci-contract-supply-chain-report.md). | Commit S07, run every hosted job, configure `main` required checks, and retain run/ruleset proof. Windows cannot substitute for Linux race/Gosec/CodeQL evidence. | `THR-009`, `THR-013`, `THR-014`, `THR-030`, `THR-042`, `THR-060` | S07 local complete; hosted enforcement S08 |
...
| `FND-023` | Partial | Backend/web images use digest-pinned bases, source labels, exact revision tags, recorded SHA-256 identity, minimal runtime files, and non-root/read-only/capability-dropped/no-new-privileges execution. | The current worktree is uncommitted and no GHCR digest is published or promoted; build from committed source and verify the same registry digest through release/promotion. | `THR-013`, `THR-060` | S07 local mechanics; hosted release S08 |
| `FND-024` | Partial | The release workflow configures digest-only keyless Cosign signatures and GitHub build/SBOM attestations, verifies expected repository/workflow identity, and has no unsigned fallback. | Run the hosted release and retain signature, provenance, SBOM attestation, tamper/wrong-source, and unavailable-signing evidence. | `THR-013`, `THR-018`, `THR-020`, `THR-060` | S07 design/config; hosted proof S08 |
...
| `FND-026` | Partial | `CODEOWNERS` covers ledger, authorization, migrations, cryptography, CI, deployment, contracts, and supply-chain locks; a static test rejects coverage drift. | Configure and evidence GitHub ruleset code-owner review, stale-approval dismissal, conversation resolution, and audited bypass constraints. | `THR-007`, `THR-013`, `THR-014`, `THR-018`, `THR-020`, `THR-040` | S07 file/policy complete; hosted enforcement S08 |
...
S07 passes its local CI-equivalent, history/security, Git-baseline contract, and supply-chain commands. Four hashed SPDX surfaces, zero reachable Govulncheck findings, complete-history Gitleaks, deleted-history/action/contract/reference canaries, critical-CVE/license gates, and hardened backend/web image checks pass. The current source is pre-commit; Windows race/Gosec are unavailable; no hosted job, ruleset, review, registry digest, signature, or provenance exists. Therefore FND-022/FND-027/FND-054 are satisfied locally, while FND-020/FND-023/FND-024/FND-026 remain partial and the full completion procedure is not claimed.
...
- GitHub Actions/GHCR and keyless release identity are now accepted by ADR 0011. Hosted ruleset/job/signature/provenance evidence still does not exist and remains an S08 gate.
...
- S07 traceability is requirement-specific. Remaining partial rows must be closed only when their hosted/clean-host/release evidence exists.
