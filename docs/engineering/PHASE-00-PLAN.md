...
| `FND-020` | Partial | S07 PR run `29928153984` and S08 PR run `29943586545` passed all five configured hosted jobs, including Linux race/Gosec, real PostgreSQL/NATS, CodeQL, contracts/history, supply chain, and the one-connection constrained-pool race test; both [S07](../../evidence/phase-00/supply-chain/S07-hosted-pr-verification.md) and [S08](../../evidence/phase-00/acceptance/S08-hosted-pr-verification.md) evidence are revision-bound. | GitHub reports no `main` ruleset. Configure required checks and retain ruleset/bypass proof before treating successful execution as enforced policy. | `THR-009`, `THR-013`, `THR-014`, `THR-030`, `THR-042`, `THR-060` | Hosted execution observed; enforcement open |
...
| `FND-026` | Partial — accepted solo deviation | `CODEOWNERS` covers every sensitive boundary. ADR 0012 and the closed solo policy prohibit manufactured independence, real data/providers/credentials, and production-readiness claims; sensitive PRs require six machine-checked attestations and fresh-context self-review. | Independent approval is unavailable and not claimed. Configure and evidence the PR/status/deletion/non-fast-forward ruleset now; obtain genuine code-owner approval before any non-synthetic, real-data/provider, second-maintainer, or production-readiness trigger. | `THR-007`, `THR-013`, `THR-014`, `THR-018`, `THR-020`, `THR-040`, `THR-060` | ADR/policy implemented; hosted ruleset open; independent review trigger-bound |
...
6. The [S08 pre-commit report](../../evidence/phase-00/acceptance/S08-phase-00-acceptance-report.md) preserves the detailed live/history/supply results. [Post-commit verification](../../evidence/phase-00/acceptance/S08-post-commit-verification.md) binds final implementation `6b09b4a` and evidence revision `431821f`, including a passing detached same-host clean clone. [Hosted PR verification](../../evidence/phase-00/acceptance/S08-hosted-pr-verification.md) binds the five successful jobs and constrained-pool race markers to exact head `10ed35b`. Independent clean-host, ruleset/review, registry/signature/provenance, encryption, alert-routing, and product-replay gaps remain explicit.
