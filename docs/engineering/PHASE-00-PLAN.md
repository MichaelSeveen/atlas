...
| `FND-021` | Satisfied | S05 live scripts require PostgreSQL 18.4 and NATS 2.14.0 JetStream containers. Real role, migration, lock, backup, and recovery commands cannot be satisfied by repository mocks; throwaway databases are cleaned up. | Wire the live lane into protected CI in S07. Duplicate/replay cases are inapplicable until an outbox/consumer exists and must be added with that behavior. | `THR-014`, `THR-015`, `THR-019`, `THR-040`, `THR-054` | S05 complete locally; enforce S07 |
...
| `FND-054` | Partial | Exact Go/pgx and Bun/React pins, frozen lockfile, checksums, reproducible install/build commands, and [update policy](TOOLCHAIN_POLICY.md) exist. | Dependency/license/scanner, base-image digest, CI-action, SBOM/provenance, and emergency-update proof remain S07. | `THR-013`, `THR-060` | S01/S04/S05 pins; complete S07 |
...
| Ten named “tests most agents skip” | S02–S05 implement named tests #3, #6, #8, and #9 plus adjacent mutation/failure cases; S08 must report all ten. | S02–S08 |
...
- CI host, deployment/secret manager, backup encryption/retention, production identity/broker/object choices, signing/provenance, and generated product-client strategy remain unresolved at their planned S06/S07/S08 boundaries.
- S05 traceability is requirement-specific. Remaining generic planned rows must be refined only when their implementing slice produces executable evidence.
