...
The pins match the reviewed S01/S04/S05 implementation environments; they are reproducibility choices, not compatibility or support-lifetime claims. CI, SBOM, provenance, immutable image digest, and automated update verification remain outstanding under `FND-020..024` and `FND-054`.
...
- Pin Go dependencies when introduced; `go.mod`/`go.sum` changes receive dependency review. S05 pins pgx/v5 only for the bounded PostgreSQL migration-state readiness probe.
