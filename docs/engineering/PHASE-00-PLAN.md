...
| `FND-010` | Partial | S04 defines the complete constrained topology; EVD-P00-S08-005 records exact full local S08 at `7b1e28e`, including an exit-zero bounded Bun shutdown, and adds ref-guarded release wiring for the same live/history/supply/clean-clone path before publication. | The full container wrapper still has not passed on the separately administered hosted Docker runner. | `THR-019`, `THR-025`, `THR-042`, `THR-045` | Exact local closure pass; hosted live run open |
...
| `FND-023` | Partial | Committed backend/web images use digest-pinned bases, source labels, exact revision tags, recorded SHA-256 identity, and hardened execution; EVD-P00-S08-005 proves the protected-ref workflow completes full S08 before registry authentication. | No GHCR digest is published or promoted; verify both registry digests through the hosted release/promotion path. | `THR-013`, `THR-060` | Mechanics and pre-publish gate pass; release open |
| `FND-024` | Partial | The ref-guarded workflow puts full live/history/supply/clean-clone S08 before every registry mutation, then configures digest-only Cosign and GitHub build/SBOM attestations with no unsigned fallback. | The hosted release run list remains empty. Run it only with separate authorization and retain signature, provenance, SBOM attestation, tamper/wrong-source, and unavailable-signing evidence. | `THR-013`, `THR-018`, `THR-020`, `THR-060` | EVD-P00-S08-005 local closure; hosted release open |
...
6. The [S08 pre-commit report](../../evidence/phase-00/acceptance/S08-phase-00-acceptance-report.md) preserves the original detailed live/history/supply results. [Post-commit verification](../../evidence/phase-00/acceptance/S08-post-commit-verification.md) binds the clean clone, [hosted PR verification](../../evidence/phase-00/acceptance/S08-hosted-pr-verification.md) binds constrained-pool race proof, [solo-governance verification](../../evidence/phase-00/acceptance/S08-solo-maintainer-governance.md) binds ruleset `19577130`, and [hosted-release closure preflight](../../evidence/phase-00/acceptance/S08-hosted-release-closure-preflight.md) binds the fail-before-publish/live-shutdown changes to implementation `eae43b6` and exact full local verification `7b1e28e`. Independent clean-host, trigger-bound independent review, registry/signature/provenance, encryption, alert-routing, and product-replay gaps remain explicit.
