...
| `FND-010` | Satisfied | EVD-P00-S08-007 records a successful complete full S08 command on fresh hosted Docker: topology, trace/outage, backup/restore, race, exit-zero shutdown, exact clone, cleanup, and outer verification all pass. | Revalidate on material environment/runtime changes; do not infer production deployment or capacity. | `THR-019`, `THR-025`, `THR-042`, `THR-045` | Fresh-host command complete |
...
| `FND-023` | Partial | EVD-P00-S08-007 records source-tagged backend/web GHCR indexes, exact index and Linux/amd64 manifest digests, hardened build gates, and 20.97/41.23 MiB compressed sizes. | Complete automated exact-source verification and explicit release-SBOM retention in one green protected run. | `THR-013`, `THR-060` | Immutable publication exists; green tail open |
| `FND-024` | Partial | EVD-P00-S08-007 independently verifies keyless signatures and one SLSA v1 plus one SPDX 2.3 attestation per digest against the exact workflow/source/ref. | Merge `4b34edd`, obtain a green automated identity transcript for both predicate types, and retain the named release SBOM artifact. | `THR-013`, `THR-018`, `THR-020`, `THR-060` | Cryptographic artifacts valid; automated tail open |
