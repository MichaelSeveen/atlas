...
- **Current slice:** [S08 — Phase 00 acceptance, restore, and evidence release](PHASE-00-PLAN.md#s08--phase-00-acceptance-restore-and-evidence-release) is extended by hosted-release closure implementation `eae43b62f3e0e3f95a09bff46f1ac73217dde5c3`. Local full-stack/recovery checks pass and the Bun container exits zero within the bounded stop; the ref-guarded release now requires full live/history/supply/clean-clone S08 before registry authentication. No hosted release, GHCR artifact, signature, or provenance exists, and independent review remains unavailable/not claimed under ADR 0012, so Phase 00 completion is not claimed.
...
- EVD-P00-S08-005 supersedes the earlier local forced-stop observation: the Bun web process now handles SIGINT/SIGTERM and the rebuilt Podman container exited `0` in `6303ms` within its eight-second deadline. Hosted Docker confirmation remains open.
