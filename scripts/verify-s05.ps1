...
try {
    ...
    if ($Live) {
        ...
        Write-Output 's05_live_verification=PASS'
    }
    else {
        Write-Output 's05_live_verification=NOT_REQUESTED'
    }
    Write-Output 's05_named_skipped_tests=8,9'
    Write-Output "source_revision=$sourceRevision"
    Write-Output 's05_verification=PASS'
}
finally {
    Pop-Location
}
