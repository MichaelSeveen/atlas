...
- Phase 01 S04 verification: `pwsh -NoProfile -File ./scripts/verify-p01-s04.ps1`; add `-Live -ContainerRuntime podman` for real PostgreSQL session/revocation/idempotency concurrency, a rebuilt synthetic Keycloak stack, customer/merchant higher-assurance rotation, workforce baseline denial, and bounded identity telemetry.
