...
Phase 00 is complete only when the relevant traceability rows and the evidence catalogue identify the exact committed source, the full acceptance command passes from a genuinely independent clean supported machine, hosted branch rules require the observed checks and active review policy, and the release workflow publishes and verifies digest-promoted signed/provenanced artifacts. ADR 0012 permits an explicitly recorded synthetic-only solo-maintainer deviation without pretending self-review is independent; genuine code-owner approval remains mandatory before any policy revalidation trigger. Until the remaining external facts exist, S08 is implemented but the Phase 00 completion claim remains blocked.
