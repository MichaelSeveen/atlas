...
| `FND-054` | Partial | Exact Go/pgx/OpenTelemetry and Bun/React pins, frozen lockfile, checksums, reproducible install/build commands, and [update policy](TOOLCHAIN_POLICY.md) exist. | Dependency/license/scanner, base-image digest, CI-action, SBOM/provenance, and automated update proof remain S07. | `THR-013`, `THR-060` | S01/S04–S06 pins; complete S07 |
...
- CI host, production secret manager/custody, backup encryption/retention, production identity/broker/object choices, signing/provenance, alert backend/routing, and generated product-client strategy remain unresolved at their planned S07/S08 boundaries.
- S06 traceability is requirement-specific. Remaining generic planned rows must be refined only when their implementing slice produces executable evidence.
