...
- Git origin is `https://github.com/MichaelSeveen/atlas.git`. Authorized hosted release run `29959302878` at main revision `36fdaa6` passed race/Gosec, supply chain, live recovery, and bounded shutdown, then failed closed during disposable Go module-cache cleanup before GHCR authentication. Correction `baf2e67` uses `-modcacherw` only for the isolated clone and has a seeded regression. No image, signature, or provenance was published; independent human review remains unavailable/not claimed and becomes mandatory at an ADR 0012 trigger.
