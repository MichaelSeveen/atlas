...
| `FND-013` | Absent | Requirement only; no reset command | Reset must resolve/print the target environment and require an exact explicit confirmation. Test wrong/missing confirmation, production-reference target rejection, and path/namespace containment before deletion. | `THR-019`, `THR-045` | S04 |
...
| `FND-030` | Absent | Environment categories are named, but no config artifacts exist | Define typed local/test/staging/production-reference config with safe defaults and drift checks. Test missing/unknown keys, cross-environment resource reuse, and production-reference fail-closed behavior. | `THR-020`, `THR-042`, `THR-045`, `THR-060` | S04 |
| `FND-031` | Absent | Key separation is policy-only in [security model §10](../atlas-prd/01-architecture/01_SECURITY_AND_TRUST_MODEL.md) | Provision distinct signing/encryption/database/identity/merchant credentials per environment and verify fingerprints never match; cover restore and rotation grace periods without exposing values. | `THR-020`, `THR-023`, `THR-045` | S04, finalized S07 |
| `FND-032` | Absent | Synthetic labels required by scope/Phase 00; no UI exists | Render persistent environment/synthetic-data labels in all non-production shells and exports/screenshots. Browser tests cover routes, responsive states, logout/back-forward cache, and production-reference misconfiguration. | `THR-045`, `THR-046`, `THR-047` | S04 |
| `FND-033` | Absent | Only requirement text exists | Add typed flag metadata (owner, expiry, default, risk, rollback) and reject unknown/expired/high-risk unsafe transitions. Tests cover stale config, concurrent evaluation, default on outage, and rollback. | `THR-024`, `THR-036`, `THR-042`, `THR-059` | S04 |
| `FND-040` | Absent | Context fields are specified in [API/event standards §9](../atlas-prd/01-architecture/03_API_AND_EVENT_STANDARDS.md) and [reliability §6](../atlas-prd/01-architecture/04_RELIABILITY_OBSERVABILITY_DR.md) | Propagate generated/validated request, correlation, trace, causation context through HTTP, transaction spans, outbox/event, worker, and simulator. Test invalid/untrusted headers, retries, and async linkage without using context as authorization. | `THR-009`, `THR-015`, `THR-030`, `THR-060` | S03 then S06 |
...
| `FND-043` | Absent | Golden trace requirement only | Create a deterministic synthetic request spanning web/API/DB/outbox/worker/simulator where available. Smoke test required spans/links, safe attributes, revision metadata, and missing-telemetry failure. | `THR-009`, `THR-030`, `THR-042`, `THR-060` | S03 then S06/S08 |
...
| `FND-053` | Absent | Controls are specified in security docs; no HTTP server exists | Add headers, body/decompression/query limits, server/client timeouts, CORS allowlist, cache policy, and safe problem errors. Test `ADV-RES-001`, slow client, wildcard credentialed CORS rejection, header scan, and internal-detail non-disclosure. | `THR-010`, `THR-041`, `THR-042`, `THR-043` | S03 |
...
|---|---|---|
| `GET /health/live`, `GET /health/ready`, `GET /version` | Absent from code and the reference OpenAPI. Contract must be updated before/with handlers; readiness must fail on migration lag without leaking topology. | S03 |
| Common OpenAPI components for problem details, request/correlation IDs, idempotency, cursor pagination, money, actor/session, ETags/versions | Money, idempotency, cursor, ETag, and Problem components exist in the reference contract; request/correlation and actor/session coverage must be reviewed and executable examples added. | S03/S07 |
...
| Eight runbook topics | No repository runbooks. | S03/S05/S06/S07 |
