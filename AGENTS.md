...
- Git origin is `https://github.com/MichaelSeveen/atlas.git`. Successful hosted release run `29964442782` at protected-main revision `9761754` passed full fresh-host S08 before publication, pushed immutable backend/web GHCR indexes, keyless-signed them, attached and automatically verified exact-source SLSA/SPDX attestations, and retained all four SPDX SBOM surfaces for 90 days. Independent exact-identity verification also passes. FND-010, FND-022, FND-023, and FND-024 are satisfied at the synthetic foundation depth; overall Phase 00 remains partial for the six explicitly deferred requirements.
