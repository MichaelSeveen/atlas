...
- Git origin is `https://github.com/MichaelSeveen/atlas.git`. S08 implementation `6b09b4a` is post-commit and same-host clean-clone verified at `431821f`. ADR 0012 solo governance is implemented at `08762a3`; PR #19 run `29949126130` passed its sensitive declaration and all five hosted jobs at `8c10323`. Active `main` ruleset `19577130` requires those checks with no bypass actors. Independent human review is unavailable/not claimed and becomes mandatory at an ADR 0012 trigger; independent clean-host and registry/signature/provenance limitations remain outstanding.
