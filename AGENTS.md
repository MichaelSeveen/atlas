...
This repository has completed **Phase 01 — Identity, access, tenancy, and privileged workforce controls** at bounded synthetic local/reference evidence depth. P01-S01 through P01-S09 are complete: the aggregate closure binds all thirty IAM rows, generated OpenAPI TypeScript shapes, customer/merchant/workforce browser boundaries, six deterministic synthetic provider subjects, support/risk/finance personas, adversarial client-state proof, inherited recovery/runtime gates, and operational exercises. Administrator membership removal and break-glass remain fail-closed; future privileged and financial actions remain reserved fail-closed. Phase 02 planning and contract/decision closure is next. Do not implement wallet or money-movement features before their prerequisite phase decisions and gates pass.

## Stack and commands discovered

- Backend: Go only. The module is `github.com/MichaelSeveen/atlas`, derived from the configured GitHub origin, with language baseline Go 1.25.0, security-patched toolchain Go 1.25.12, gRPC 1.82.1 for the indirect OTLP transport after the `GO-2026-6061` nightly remediation, and go-redis/v9 9.21.0 for reconstructible credential rate counters. Separate `api`, feature-free `worker`, and feature-free `simulator` entry points exist. The API exposes the three operational routes plus seventeen approved S04/S05 identity/session/organization, six S07 approval, and four S08 credential lifecycle operations; `GET /v1/me` also accepts the bounded AtlasKey machine principal. Worker and simulator remain inert.
...
- Phase 01 S04 verification: `pwsh -NoProfile -File ./scripts/verify-p01-s04.ps1`; add `-Live -ContainerRuntime podman` for real PostgreSQL self/admin revocation and idempotency concurrency, Audit-outage rollback, a rebuilt synthetic Keycloak stack, customer/merchant higher-assurance rotation, workforce baseline denial, account-enumeration differential, and bounded identity telemetry.
- Phase 01 S05 verification: `pwsh -NoProfile -File ./scripts/verify-p01-s05.ps1`; add `-Live -ContainerRuntime podman` for the real PostgreSQL tenant, invitation/acceptance, role-change/removal race, Unicode projection collision, session invalidation, and Audit-outage checks.
- Phase 01 S06 verification: `pwsh -NoProfile -File ./scripts/verify-p01-s06.ps1`; add `-Live -ContainerRuntime podman` for real PostgreSQL concealment timing, purpose-bound masked disclosure, Audit rollback, and two-pool role/resource/assurance invalidation.
- Phase 01 S07 verification: `pwsh -NoProfile -File ./scripts/verify-p01-s07.ps1`; add `-Live -ContainerRuntime podman` for migration/recovery regression plus real PostgreSQL maker-checker separation, reauthorization, terminal-state, replay, tamper, concurrency, and Audit-outage proof.
- Phase 01 S08 verification: `pwsh -NoProfile -File ./scripts/verify-p01-s08.ps1`; add `-Live -ContainerRuntime podman` for migration/recovery regression plus real PostgreSQL one-time-secret replay, concurrent rotation/revocation, Audit rollback, and real Redis credential/tenant/network limits.
- Phase 01 aggregate/closure verification: `pwsh -NoProfile -File ./scripts/verify-p01.ps1`; add `-Live -ContainerRuntime podman -Browser msedge` for the inherited full stack/PITR gate, empty-state six-subject Keycloak rebuild, customer/merchant/support/risk/finance journeys, real-provider browser login/logout, and adversarial client-state acceptance. Add `-History` and `-SupplyChain` when those inherited gates are required and their tooling is available.
...
- S04 composes local PostgreSQL, Redis, NATS, MinIO, the OTel Collector, Keycloak, API, worker, simulator, and web. S05 adds repository-owned PostgreSQL migration/role/recovery controls. P01-S03 adds `atlas_identity`/`atlas_audit` persistence, deterministic synthetic product seeds, explicit tenant-query foundations, insert-only Audit grants, and product-state PITR revalidation. P01-S04 composes only the approved OIDC BFF and durable application-session boundary against those stores; P01-S05 adds the approved organization/tenancy operations; P01-S06 adds only the bounded current authorization evaluator and masked member-field path; P01-S07 adds only the typed approval aggregate and sole approved membership-role target action; P01-S08 adds only the ratified `identity:read` merchant credential boundary and ephemeral abuse counters; P01-S09 adds generated-contract React identity/operations flows, synthetic acceptance personas, adversarial browser proof, and aggregate closure controls. Phase 00 S06 exports bounded traces/metrics from the Go processes, emits source-redacted JSON logs, and keeps the collector outside authoritative readiness; Phase 00 S07/S08 retain their CI/supply-chain/acceptance meanings. These remain synthetic local/reference controls only. No broker stream, worker job, managed production secret provider, financial credential scope, or financial behavior exists yet.
- Git origin is `https://github.com/MichaelSeveen/atlas.git`. Successful hosted release run `29964442782` at protected-main revision `9761754` passed full fresh-host S08 before publication, pushed immutable backend/web GHCR indexes, keyless-signed them, attached and automatically verified exact-source SLSA/SPDX attestations, and retained all four SPDX SBOM surfaces for 90 days. Independent exact-identity verification also passes. ADR 0013 closes Phase 00 for the synthetic feature-free foundation with 34 satisfied requirements, the ADR 0012 accepted deviation for FND-026, and accepted scope decisions for FND-040/FND-042. ADR 0014 owns the Phase 01 identity/access contract boundary; ADR 0015 additively closes administrator session-revocation transport; ADR 0016 additively closes invitation-acceptance bootstrap authentication. All thirty Phase 01 IAM requirements are Verified at current bounded scope; `EVD-P01-S09-CLOSURE` owns the aggregate closure and limitations.
