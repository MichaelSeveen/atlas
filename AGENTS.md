...
This repository is currently in **Phase 01 — Identity, access, tenancy, and privileged workforce controls**. P01-S01 through P01-S07 are complete at their bounded evidence depth. S07 adds an Operations-owned typed maker-checker aggregate with immutable Atlas-principal separation, RFC 8785/SHA-256 payload binding, dynamic checker eligibility, execution-time authorization/step-up/target revalidation, terminal-state denial, safe replay, atomic Identity/Audit effects, and real PostgreSQL concurrency/tamper/failure proof for the sole non-financial `identity.organization.membership.change_admin` action. P01-S08 merchant API credentials is next. Administrator membership removal remains fail-closed pending exact fresh-step-up and last-administrator policy. Do not implement wallet or money-movement features before their prerequisite phases pass.

## Stack and commands discovered

- Backend: Go only. The module is `github.com/MichaelSeveen/atlas`, derived from the configured GitHub origin, with language baseline Go 1.25.0, security-patched toolchain Go 1.25.12, and gRPC 1.82.1 for the indirect OTLP transport after the `GO-2026-6061` nightly remediation. Separate `api`, feature-free `worker`, and feature-free `simulator` entry points exist. The API exposes the three operational routes plus seventeen approved S04/S05 identity/session/organization and six S07 approval operations through application boundaries; worker and simulator remain inert.
