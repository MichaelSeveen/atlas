...
- **Current slice:** `P01-S06` deny-by-default authorization, purpose-bound masking, decision Audit, enumeration resistance, and PostgreSQL invalidation is complete at its bounded evidence depth. `EVD-P01-S06-AUTHORIZATION-CLOSURE` binds the cumulative static/live gate, exact policy parity, unknown/stale mutation matrix, real PostgreSQL concealment timing, Audit rollback, two-pool invalidation, observability canaries, and response procedure. The implemented authorization action is intentionally limited to the current organization-member list; Phase 01 defines no search/suggestion/autocomplete operation. `P01-S07` typed maker-checker approval foundation and execution-time reauthorization is next.
- **Implementation state:** Phase 00 remains historically complete for its bounded synthetic feature-free foundation, P01-S03 consumed the `first-product-schema`/`first-product-durable-state` triggers, and the runtime exposes the nine approved S04 identity/session operations plus eight S05 organization/tenancy operations alongside the three operational routes. Merchant sessions validate their explicit selected tenant even when a principal has multiple active memberships; switching rechecks current and target authority in one PostgreSQL transaction, revokes the old verifier with zero grace, creates a CSRF-rebound session without extending its absolute lifetime, and records Audit atomically. Member listing rechecks authoritative PostgreSQL session, membership, exact role permissions, tenant, action, object, field, purpose, assurance, authorization version, and resource state before page or cursor work; it returns at most 100 rows with no total count, projects the optional invitation-derived email hint as null by default, and permits only its already-masked value for the exact sensitive permission/purpose with atomic Audit. Known-foreign and absent tenant identifiers share one concealed response path with bounded timing. Two independent API pools immediately reject stale role, disabled-resource, and assurance facts without Redis or process-local decision truth. Invitation issuance/acceptance and direct viewer/operator role changes/removals retain their S05 single-use, idempotency, zero-grace invalidation, race, and Audit-rollback guarantees; PostgreSQL lock-timeout races now join serialization/deadlock failures in the bounded retry classifier. Administrator-involved role transitions and administrator removal remain fail-closed pending S07 typed maker-checker plus exact fresh-step-up/last-administrator policy. No approval execution, API credential, event, worker job, financial workflow, managed production secret provider, or wallet UI exists.
...
| Application code | Go module `github.com/MichaelSeveen/atlas`; `cmd/api` serves liveness, readiness, version, the nine approved S04 identity/session operations, and eight S05 organization/tenancy operations through Identity and Audit application boundaries, while worker/simulator remain feature-free. Identity owns typed tenant context, the source-controlled deny-default evaluator, exact PostgreSQL role-permission rechecks, purpose/field masks, application-session authority, organization visibility, invitations/acceptance, and direct viewer/operator membership mutations; Audit owns validated event/application interfaces and caller-transaction PostgreSQL recording. `cmd/dbctl` validates thirteen released migrations and `cmd/contractctl` lints/compares canonical contracts. No generated product client exists. |
| Tooling | Go 1.25.12 with language baseline 1.25.0, pgx/v5 5.10.0, OpenTelemetry Go 1.43.0, gRPC 1.82.1, Bun 1.3.0, and React 19.2.7 are pinned; `bun.lock` is frozen. `verify-p01-s06.ps1` composes the S04/S05 regression chain with exact authorization policy/mutation/contract/architecture checks and, under `-Live`, real PostgreSQL concealment timing, purpose-bound masking, Audit rollback, membership races, and two-pool role/resource/assurance invalidation. GitHub Linux supplied the prior Phase 00 race/Gosec/CodeQL and constrained-pool evidence. |
...
| Classification | Count | Requirement IDs |
|---|---:|---|
| Verified | 18 | `IAM-001`, `IAM-003..005`, `IAM-007`, `IAM-010..015`, `IAM-020..026` |
| Planned | 12 | `IAM-002`, `IAM-006`, `IAM-030..034`, `IAM-040..044` |

These eighteen rows are verified only for their explicit S04 application-session, S05 tenancy,
and S06 current authorization scopes. Phase 01 remains open; approval, credential, and final
