...
CREATE TEMP TABLE atlas_phase01_policy_seed_v4_document (
    payload jsonb NOT NULL,
    seed_checksum character(64) NOT NULL
) ON COMMIT DROP;

INSERT INTO atlas_phase01_policy_seed_v4_document(payload, seed_checksum)
...
BEGIN
    ...
    FROM atlas_phase01_policy_seed_v4_document;
    IF document->>'seed_id' <> 'atlas-phase01-identity-policy-v4'
       OR document->>'predecessor_seed_id' <> 'atlas-phase01-identity-policy-v3'
       ...
       OR document_checksum !~ '^[0-9a-f]{64}$' THEN
        RAISE EXCEPTION 'Phase 01 policy seed v4 identity or checksum is invalid';
    END IF;
END
$seed_validation$;
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE seed_id = document->>'predecessor_seed_id'
          AND seed_checksum = '0f57d9cc63ae9f85bb50bf4cc9c409ca2355e4d4529441ca37c9a73baafd92fa'
        ...
        RAISE EXCEPTION 'Phase 01 policy seed v4 predecessor state is invalid';
    END IF;
END
...
FROM atlas_phase01_policy_seed_v4_document AS document
...
BEGIN
    ...
    IF (
        ...
        WHERE seed_id = document->>'seed_id'
          AND seed_checksum = (
              SELECT seed_checksum FROM atlas_phase01_policy_seed_v4_document
          )
          AND policy_checksum = document->>'policy_sha256'
    ) THEN
        RAISE EXCEPTION 'Phase 01 policy seed v4 application drift detected';
    END IF;
END
$seed_application$;
