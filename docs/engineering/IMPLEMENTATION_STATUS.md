...
- **Current slice:** `P01-S08` merchant API credentials, one-time secret handling, rotation, and scoped anomaly controls is complete at its bounded evidence depth. `EVD-P01-S08-CREDENTIAL-CLOSURE` binds migration 15, policy seed v4, verifier-only storage, redacted replay, exact machine bindings, savepoint-safe lifecycle concurrency, immediate revocation, real Redis multi-dimensional limits/fallback, bounded telemetry, recovery, and runbook evidence. `P01-S09` frontend flows, operational proof, adversarial acceptance, and phase closure is next.
- **Implementation state:** Phase 00 remains historically complete for its bounded synthetic feature-free foundation, P01-S03 consumed the `first-product-schema`/`first-product-durable-state` triggers, and the runtime exposes nine S04 identity/session operations, eight S05 organization/tenancy operations, six S07 approval operations, and four S08 credential lifecycle operations alongside the three operational routes. `GET /v1/me` also accepts the strict `AtlasKey` machine scheme but rejects ambiguous cookie-plus-key identity. PostgreSQL validates credential verifier/status/tenant/environment/audience/scope/expiry on every request; Redis holds only reconstructible credential/tenant/network counters and outage enters stricter bounded local limits. Create and rotate disclose a 256-bit secret once, while replay/list never recover it. Rotation has a ten-minute overlap and explicit revocation; concurrent losing claims are savepoint-rolled back while the denied Audit fact commits. Administrator membership removal remains fail-closed pending exact fresh-step-up and last-administrator policy. No event, worker job, financial credential scope, financial workflow, managed production secret provider, credential-management UI, or wallet UI exists.
...
| Application code | Go module `github.com/MichaelSeveen/atlas`; `cmd/api` serves liveness, readiness, version, 21 S04/S05/S08 Identity operations, and six S07 approval operations, while worker/simulator remain feature-free. Identity owns session/tenant/authorization/credential truth and the approved membership-role target command; Operations owns approval state and execution protocol; Audit owns validated caller-transaction recording. `cmd/dbctl` validates fifteen released migrations and `cmd/contractctl` lints/compares canonical contracts. No generated product client exists. |
| Tooling | Go 1.25.12 with language baseline 1.25.0, pgx/v5 5.10.0, go-redis/v9 9.21.0, OpenTelemetry Go 1.43.0, gRPC 1.82.1, Bun 1.3.0, and React 19.2.7 are pinned; `bun.lock` is frozen. `verify-p01-s08.ps1` composes the S04–S07 regression chain with credential model/contract/architecture/observability/evidence/build checks and, under `-Live`, real PostgreSQL lifecycle concurrency/replay/revocation/Audit rollback plus real Redis three-dimensional limits. GitHub Linux supplied the prior Phase 00 race/Gosec/CodeQL and constrained-pool evidence. |
...
| Classification | Count | Requirement IDs |
|---|---:|---|
| Verified | 28 | `IAM-001`, `IAM-003..005`, `IAM-007`, `IAM-010..015`, `IAM-020..026`, `IAM-030..034`, `IAM-040..044` |
| Planned | 2 | `IAM-002`, `IAM-006` |

These twenty-eight rows are verified only for their explicit S04 application-session, S05 tenancy,
S06 current authorization, S07 typed approval, and S08 merchant-credential scopes. Phase 01 remains
open; S09 must close the final acceptance journeys and give an honest disposition for `IAM-002` and
`IAM-006` without reinterpreting earlier evidence.
