...
psql -X -h 127.0.0.1 -U atlas_migration -d "$ATLAS_POSTGRES_DB" -v ON_ERROR_STOP=1 \
  -c "BEGIN; LOCK TABLE atlas_identity.memberships IN ACCESS EXCLUSIVE MODE; SELECT pg_sleep(3); COMMIT;" >"$holder_output" 2>&1 &
holder_pid=$!
sleep 1
...
if PGOPTIONS='-c lock_timeout=500ms -c statement_timeout=5s' psql -X -h 127.0.0.1 -U atlas_migration -d "$ATLAS_POSTGRES_DB" -v ON_ERROR_STOP=1 \
  -1 -c 'ALTER TABLE atlas_identity.memberships ADD COLUMN lock_canary text' >"$blocked_output" 2>&1; then
...
column_count="$(psql -X -h 127.0.0.1 -U atlas_migration -d "$ATLAS_POSTGRES_DB" -Atqc "SELECT count(*) FROM information_schema.columns WHERE table_schema='atlas_identity' AND table_name='memberships' AND column_name='lock_canary'")"
[ "$column_count" = '0' ]
