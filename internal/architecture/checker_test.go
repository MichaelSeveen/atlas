...
func TestFrontendToolchainPolicy(t *testing.T) {
	...
	const openAPICommand = "bunx --bun openapi-typescript ../../docs/atlas-prd/03-contracts/openapi.yaml -o ./src/generated/openapi.ts --immutable"
	if manifest.Scripts["generate:api"] != openAPICommand {
		t.Errorf("frontend OpenAPI generation must be deterministic and canonical, got %q", manifest.Scripts["generate:api"])
	}
	if manifest.Scripts["check:api"] != openAPICommand+" --check" {
		t.Errorf("frontend OpenAPI drift check must be deterministic and canonical, got %q", manifest.Scripts["check:api"])
	}
	...
	generatedPath := filepath.Join(root, "apps", "web", "src", "generated", "openapi.ts")
	generated, err := os.ReadFile(generatedPath)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(string(generated), "This file was auto-generated by openapi-typescript") ||
		!strings.Contains(string(generated), `readonly Principal: {`) ||
		!strings.Contains(string(generated), `readonly APICredential: {`) {
		t.Error("generated frontend contract is missing its generator provenance or Phase 01 schemas")
	}
	clientPath := filepath.Join(root, "apps", "web", "src", "api.ts")
	client, err := os.ReadFile(clientPath)
	if err != nil {
		t.Fatal(err)
	}
	clientText := string(client)
	if !strings.Contains(clientText, `import type {components} from "./generated/openapi";`) ||
		!strings.Contains(clientText, `components["schemas"]["Principal"]`) {
		t.Error("frontend API boundary must consume generated canonical OpenAPI types")
	}
	for _, forbiddenModel := range []string{"export type Principal = {", "export type Session = {", "export type Organization = {", "export type Credential = {", "export type Approval = {"} {
		if strings.Contains(clientText, forbiddenModel) {
			t.Errorf("frontend API boundary contains a hand-edited duplicate contract model: %s", forbiddenModel)
		}
	}
}
