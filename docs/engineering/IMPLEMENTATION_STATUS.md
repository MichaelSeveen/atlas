...
| Application code | Go module `github.com/MichaelSeveen/atlas`; `cmd/api` serves liveness, readiness, version, the nine approved S04 identity/session operations, and seven S05 organization/tenancy operations through Identity and Audit application boundaries, while worker/simulator remain feature-free. Identity owns typed tenant context, an immutable identity seed plus additive policy seed chain, OIDC verification, encrypted server-side login/session state, scoped step-up and revocation replay state, population lifetimes, rotation, self revocation, workforce security revocation, organization visibility, active-tenant rotation, masked member listing, invitation issuance, invitation-bound bootstrap authentication, atomic recipient acceptance, and direct viewer/operator member role changes; Audit owns validated event/application interfaces and caller-transaction PostgreSQL recording. `cmd/dbctl` validates eleven released migrations and `cmd/contractctl` lints/compares canonical contracts. No generated product client exists. |
| Tooling | Go 1.25.12 with language baseline 1.25.0, pgx/v5 5.10.0, OpenTelemetry Go 1.43.0, gRPC 1.82.1, Bun 1.3.0, and React 19.2.7 are pinned; `bun.lock` is frozen. gRPC 1.82.1 closes the reachable `GO-2026-6061` call paths that caused the scheduled History gate to fail on the prior main revision. `verify-p01-s04.ps1` composes prior Phase 01 regression with migration/seed integrity, real PostgreSQL roles/recovery, disposable self/admin revocation concurrency and Audit-outage rollback, rebuilt synthetic Keycloak flows, a nine-sample interleaved known/absent-user timing matrix for each realm, HTTP negative matrices, browser navigation/storage proof, and bounded identity telemetry. GitHub Linux supplied the prior Phase 00 race/Gosec/CodeQL and constrained-pool evidence. |
...
- `FND-003` — API, worker, and provider-simulator Go entry points build independently; only the API has a runtime lifecycle, now serving the three operational and sixteen approved identity/session/organization endpoints.
