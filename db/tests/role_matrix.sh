...
run_sql atlas_api "$ATLAS_POSTGRES_API_PASSWORD" "BEGIN; INSERT INTO atlas_identity.admin_session_revocation_requests(revocation_request_id, actor_principal_id, actor_session_id, target_session_id, idempotency_key_sha256, request_sha256, purpose, reason_code, outcome, current_revoked, decision_id, committed_at) VALUES ('asr_01JAT1AS00000000000991', 'usr_01JAT1AS00000000000003', 'ses_01JAT1AS00000000000901', 'ses_01JAT1AS00000000000991', decode(repeat('88', 32), 'hex'), decode(repeat('99', 32), 'hex'), 'security_review', 'compromised_session', 'not_found', false, 'dec_01JAT1AS00000000000991', CURRENT_TIMESTAMP); ROLLBACK;" >/dev/null
...
expect_denied atlas_worker "$ATLAS_POSTGRES_WORKER_PASSWORD" 'SELECT count(*) FROM atlas_identity.admin_session_revocation_requests' worker-admin-revocation-replay-read
expect_denied atlas_reporting_read "$ATLAS_POSTGRES_REPORTING_PASSWORD" 'SELECT count(*) FROM atlas_identity.admin_session_revocation_requests' reporting-admin-revocation-replay-read
...
expect_denied atlas_api "$ATLAS_POSTGRES_API_PASSWORD" "UPDATE atlas_operations.approvals SET payload_canonical = decode('00', 'hex') WHERE approval_id = 'apr_01JAT1AS00000000000001'" api-approval-payload-update
...
expect_denied atlas_api "$ATLAS_POSTGRES_API_PASSWORD" "UPDATE atlas_identity.admin_session_revocation_requests SET outcome = 'executed' WHERE revocation_request_id = 'asr_01JAT1AS00000000000991'" api-admin-revocation-replay-update
expect_denied atlas_api "$ATLAS_POSTGRES_API_PASSWORD" "DELETE FROM atlas_identity.admin_session_revocation_requests WHERE revocation_request_id = 'asr_01JAT1AS00000000000991'" api-admin-revocation-replay-delete
...
[ "$(bootstrap_sql "SELECT count(*) FROM information_schema.role_table_grants WHERE grantee = 'PUBLIC' AND table_schema IN ('atlas_identity', 'atlas_operations', 'atlas_audit')")" = '0' ]
