...
- **Current slice:** [S08 — Phase 00 acceptance, restore, and evidence release](PHASE-00-PLAN.md#s08--phase-00-acceptance-restore-and-evidence-release) is extended by hosted-release closure implementation `eae43b62f3e0e3f95a09bff46f1ac73217dde5c3`. Exact full local S08 passed from clean committed revision `7b1e28ed8e52bc44d593b6114372c28782c8468a`, including history, supply chain, live recovery, bounded exit-zero Bun shutdown, and clean-clone replay; the ref-guarded release requires that same full preflight before registry authentication. No hosted release, GHCR artifact, signature, or provenance exists, and independent review remains unavailable/not claimed under ADR 0012, so Phase 00 completion is not claimed.
...
- `FND-010` is partial: the complete constrained stack, readiness, restart, teardown, smoke, and exit-zero bounded Bun shutdown pass through local Podman; the release workflow now requires the same live S08 path on fresh hosted Docker before publication, but that hosted run has not executed.
...
- `FND-023` is partial: committed backend/web image mechanics are source-labeled, digest-recorded, non-root, read-only, capability-dropped, and critical-CVE clean; the protected-ref workflow runs full live acceptance before registry authentication, but no GHCR digest has been published or promoted.
- `FND-024` is partial: the ref-guarded release fails closed, puts full S08 before every registry mutation, and configures digest-only keyless Cosign plus GitHub build/SBOM attestations and verification; no hosted release artifact, signature, or provenance statement exists yet.
...
S08 static/live/history/supply acceptance is preserved in EVD-P00-S08-001, committed static/clean-clone acceptance passes in EVD-P00-S08-002, hosted race execution passes in EVD-P00-S08-003, solo-governance/ruleset proof passes in EVD-P00-S08-004, and EVD-P00-S08-005 binds the local hosted-release closure preflight to implementation `eae43b6` with exact full local verification at `7b1e28e`. Independent-machine, independent-review, registry, signature, and provenance proof remain absent; overall Phase 00 completion is not claimed.
...
- EVD-P00-S08-005 supersedes the earlier local forced-stop observation: the Bun web process now handles SIGINT/SIGTERM and the exact full S08 run at `7b1e28e` observed the rebuilt Podman container exit `0` in `6349ms` within its eight-second deadline. Hosted Docker confirmation remains open.
