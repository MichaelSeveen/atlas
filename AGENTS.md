...
- Git origin is `https://github.com/MichaelSeveen/atlas.git`. Hosted-release closure implementation `eae43b6` adds full live S08 before every release mutation; exact full local verification `7b1e28e` includes an exit-zero bounded Bun shutdown. Active `main` ruleset `19577130` requires the five PR checks with no bypass actors. No release has been triggered, so independent clean-host and registry/signature/provenance proof remain outstanding; independent human review is unavailable/not claimed and becomes mandatory at an ADR 0012 trigger.
