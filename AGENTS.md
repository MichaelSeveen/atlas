...
This repository is currently in **Phase 01 — Identity, access, tenancy, and privileged workforce controls**. P01-S01 planning, P01-S02 contract/decision closure, and P01-S03 identity/audit persistence plus recovery revalidation are complete in the current pre-commit worktree. P01-S04 synthetic OIDC BFF/session lifecycle is underway: the core customer/merchant session boundary and workforce fail-closed baseline exist, while idempotent step-up execution, live higher-assurance completion, admin security revocation, and remaining browser/differential evidence are still open. Do not implement wallet or money-movement features before their prerequisite phases pass.
