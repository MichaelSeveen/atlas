...
This repository is currently in **Phase 01 — Identity, access, tenancy, and privileged workforce controls**. P01-S01 through P01-S08 are complete at their bounded evidence depth. S08 adds PostgreSQL-authoritative merchant AtlasKey credentials with one-time 256-bit secret disclosure, verifier-only storage, exact tenant/environment/audience/scope/status checks on every request, ten-minute rotation overlap, explicit revocation, savepoint-safe concurrent lifecycle idempotency, ephemeral three-dimensional Redis rate limits with stricter bounded local fallback, and bounded anomaly operations. P01-S09 frontend flows, adversarial acceptance, and phase closure is next. Administrator membership removal remains fail-closed pending exact fresh-step-up and last-administrator policy. Do not implement wallet or money-movement features before their prerequisite phases pass.
